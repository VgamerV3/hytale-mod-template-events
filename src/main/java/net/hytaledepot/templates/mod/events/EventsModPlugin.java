...
public final class EventsModPlugin extends JavaPlugin {
  ...
  private final EventsModTemplate service = new EventsModTemplate();
  private final AtomicLong heartbeatTicks = new AtomicLong();
  private final ScheduledExecutorService scheduler =
      Executors.newSingleThreadScheduledExecutor(
          runnable -> {
            Thread thread = new Thread(runnable, "hd-events-mod-worker");
  ...
  public EventsModPlugin(JavaPluginInit init) {
    super(init);
  }
  ...
  protected void setup() {
    ...
    getCommandRegistry().registerCommand(new EventsModStatusCommand());
    getCommandRegistry().registerCommand(new EventsModDemoCommand());

    getEventRegistry().register(PlayerConnectEvent.class, event -> {
      service.runAction(event.getPlayerRef().getUsername(), "event-probe", heartbeatTicks.get());
    });
        ...
        scheduler.scheduleAtFixedRate(
            () -> {
              try {
                ...
                if (tick % 60 == 0) {
                  getLogger().atInfo().log("[EventsMod] heartbeat=%d", tick);
                }
              } catch (Exception exception) {
                lifecycle = Lifecycle.FAILED;
                service.incrementErrorCount();
                getLogger().atInfo().log("[EventsMod] heartbeat failed: %s", exception.getMessage());
              }
            },
            1,
            1,
  ...
  private final class EventsModStatusCommand extends CommandBase {
    private EventsModStatusCommand() {
      super("hdeventsmodstatus", "Shows runtime status for EventsModPlugin.");
  ...
  private final class EventsModDemoCommand extends CommandBase {
    private EventsModDemoCommand() {
      super("hdeventsmoddemo", "Runs a demo action for EventsModPlugin.");
    setAllowsExtraArguments(true);
      this.setPermissionGroup(GameMode.Adventure);
    }
